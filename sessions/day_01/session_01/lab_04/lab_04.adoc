...
----
+
NOTE: The path `../../../../../.git` refers to the `.git` directory at the root of the course materials.  If it's easier
use the absolute path: `$COURSE_HOME/.git` instead.
...
----
+
These will add the project's Maven coordinates to the `/info` endpoint. The Spring Boot Maven plugin will cause them to automatically be replaced in the assembled JAR.  If you are doing this in your IDE, you may get an error due to the use of the `@` symbol - please
ignore.
