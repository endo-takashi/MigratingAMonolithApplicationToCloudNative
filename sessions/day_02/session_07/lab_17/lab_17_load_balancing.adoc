...
. A completed `springtrader-config-server` project has been placed in `$COURSE_HOME/day_02/session_07/lab_17/initial/lab17-springtrader-config-server` for your convenience.
...
$ mvn clean package
$ java -jar target/lab17-springtrader-config-server-0.0.1-SNAPSHOT.jar
----
...
$ mvn clean package
$ java -jar target/lab17-springtrader-eureka-0.0.1-SNAPSHOT.jar
----

. A completed `springtrader-producer` project has been placed in `$COURSE_HOME/day_02/session_06/lab_17/initial/springrader-producer` for your convenience.  Rebuild manually:
+
----
...
. You now need two separate terminal windows so we can run two instances of the producer:
+
----
Linux:
...
. In the second window we need to use a different port and give the producer a different instance id:
----
Linux:
